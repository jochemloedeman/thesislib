...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Union[
        ...
        Tuple['torch.Tensor[num_frames, channels, height, width]', Union[
            int, List[int]]],
    ...
    def _get(self, record: VideoRecord,
             frame_start_indices: 'np.ndarray[int]') -> Union[
        Tuple[List[Image.Image], Union[int, List[int]]],
        Tuple['torch.Tensor[num_frames, channels, height, width]', Union[
            int, List[int]]],
        Tuple[Any, Union[int, List[int]]],
    ...
    def _remove_corruptions(self):
        new_video_list = [
            ...
            and os.path.basename(record.path) not in ('99996', '82682')
        ]
...
class ImglistToTensor(torch.nn.Module):
    ...
    @staticmethod
    def forward(img_list: List[
        Image.Image]) -> 'torch.Tensor[NUM_IMAGES, CHANNELS, HEIGHT, WIDTH]':
        """
        ...
        """
        return torch.stack(
            [transforms.functional.to_tensor(pic) for pic in img_list])
