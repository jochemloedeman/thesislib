...
from torchvision.datasets.kinetics import _dl_wrap
...
class Kinetics(VisionDataset):
    ...
    def __init__(
        ...
        # TODO: support test
        self.num_classes = verify_str_arg(num_classes, arg="num_classes",
        ...
        if _legacy:
            ...
            if download:
                raise ValueError(
                    "Cannot download the videos using legacy_structure.")
        else:
            self.split_folder = path.join(root, split)
            self.split = verify_str_arg(split, arg="split",
    ...
    def download_and_process_videos(self) -> None:
        """Downloads all the videos to the _root_ folder in the expected
        format. """
        tic = time.time()
    ...
    def _download_videos(self) -> None:
        ...
        with open(split_url_filepath) as file:
            list_video_urls = [urllib.parse.quote(line, safe="/,:") for line in
    ...
    def _make_ds_structure(self) -> None:
        ...
        if not check_integrity(path.join(annotation_path, f"{self.split}.csv")):
            download_url(self._ANNOTATION_URLS[self.num_classes].format(
                split=self.split), annotation_path)
        ...
        with open(annotations) as csvfile:
            ...
            for row in reader:
                ...
                label = row["label"].replace(" ", "_").replace("'", "").replace(
                    "(", "").replace(")", "")
    ...
    def _remove_corruptions(self):
        with open(os.path.join(self.root, 'corrupt.txt')) as corrupt_file:
            corrupt_paths = corrupt_file.readlines()
            corrupt_paths = [path.strip() for path in corrupt_paths]

        new_samples = [sample for sample in self.samples
                       if sample[0] not in corrupt_paths]
